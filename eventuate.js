function upsertParagraph(div, id, content) {
  ...
  if (existingParagraph) {
    existingParagraph.innerHTML = content;
  } else {
...
const volunteersExtract = document
  ...
  .nextElementSibling.querySelector("p").innerHTML;
...
for (const [section, content] of Object.entries(reportDetails)) {
  if (content.details) {
    const paragraphText = `<b>${content.title}:</b> ${content.details}.`;
